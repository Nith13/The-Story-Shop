...
public Cart(String uEmail) {
        ...
        public void actionPerformed(ActionEvent event) {
       	 try {
	        	 ...
	        	 while(rs.next()) {
	        		 ...
	        		 if(x>1) {
	        			 --x;
	        			 query = "update cart set item_count="+x+" where item_name='"+name+"'";
	        		 }else {
	        			 query = "delete from cart where item_name = '"+name+"'";
	        		 }
	        		 stmt.executeUpdate(query);
	        		 Cart obj = new Cart(uEmail);   
   ...
   JPanel panel = new JPanel();
	...
	JLabel lblNewLabel_4 = new JLabel("The Story Shop\r\n");
	lblNewLabel_4.setHorizontalAlignment(SwingConstants.CENTER);
	lblNewLabel_4.setFont(new Font("Tahoma", Font.PLAIN, 20));
	lblNewLabel_4.setBounds(566, 19, 332, 33);
	...
	btnNewButton.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent e) {
			if(chkAgree.isSelected()) {
				dispose();
				new summary(uEmail).setVisible(true);
			}
			else {
	...
	try {
		...
		int heightB= 70, heightP =70 , heightC =70, heightT =70, heightD = 70;
		...
		while(rs.next()) {
			...
			btn.setBounds(500, heightD, 106, 32);
			btn.setToolTipText("Delete Item");
